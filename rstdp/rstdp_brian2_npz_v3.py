...
def parse_args():
    p = argparse.ArgumentParser(description="R-STDP (pop outputs) with epoch subset, logging, and multi-readouts")
    # Data
...
def train_and_eval(X, y, C, T, args):
    ...
    n_train = int(np.floor(args.train_split * n))
    ...
    for ep in range(args.epochs):
        ...
        print(f"           [Val] acc | count: {accc:.3f} | weighted(tau={args.readout_tau:g}): {accw:.3f} | first: {accf:.3f} | race-N(N={args.race_N}): {accr:.3f} on {val_total} samples")
