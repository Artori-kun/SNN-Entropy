#!/usr/bin/env python3
# Unsupervised STDP with inhibition + homeostasis.
# No fixed population-to-class mapping. After each epoch, we infer the best
# mapping pop->{0,1} from the *training subset* (count-based winner), then
# compute Train/Val accuracy using that mapping. Also logs per-epoch |Δw|.
#
# Example:
# python stdp_unsup_inh_npz_v3.py --npz-dir spike_data/syn --channels 32 --time 100 \
...
# ---------- Count-only readout with mapping ----------
...
def main():
    ...
    # logging
    ap.add_argument("--save", type=str, default="unsup_wlog.npz")
    ...
    for ep in range(1, args.epochs+1):
        ...
        # ---- infer mapping from training subset ----
        ...
        # Homeostasis (epoch-level): adjust v_th to hit target activity range
