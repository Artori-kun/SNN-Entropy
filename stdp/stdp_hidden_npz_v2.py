...
# -*- coding: utf-8 -*-
"""
STDP (unsupervised) HIDDEN layer + population OUTPUT classifier — v2
Key improvements to avoid 0.5 plateau:
- Time-weighted class preference for hidden neurons (earlier spikes count more)
- EMA (exponential moving average) of preference across epochs for stability
- Preference threshold (dead-zone) to suppress ambiguous neurons in mapping
- Optional random delays on Input->Hidden synapses to add temporal diversity
- Stronger output jitter to break symmetry
- Hidden activity diagnostics each epoch

Pipeline:
  Train subset each epoch with STDP active (until pretrain_epochs), accumulate hidden preferences (time-weighted).
  Update H->Out weights from EMA'd preferences (with threshold), evaluate on Validation (STDP frozen).
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="STDP hidden + population output (v2)")
    # Data
    ...
    p.add_argument("--w_out_gain", type=float, default=0.9)
    p.add_argument("--jitter_out", type=float, default=0.10)
    p.add_argument("--tau_pref", type=float, default=40.0, help="Tau (ms) for time-weighting hidden spikes when building preference")
    p.add_argument("--theta_pref", type=float, default=0.10, help="Dead-zone threshold for preference magnitude")
    p.add_argument("--pref_ema_alpha", type=float, default=0.2, help="EMA factor for preference across epochs")
    ...
    p.add_argument("--report", type=str, default="./stdp_hidden_v2_report")
    return p.parse_args()
...
def build_network(C, T, dt_ms, args, rng):
    ...
    # Optional heterogeneous delays to add temporal diversity
    ...
    v_th : 1
    '''
    G_out = NeuronGroup(Nout, eqs_o, threshold='v>v_th', reset='v=0',
...
def train_and_eval(X, y, C, T, args):
    ...
    def update_out_weights_from_pref(pref):
        # dead-zone threshold and scaling
        theta = float(args.theta_pref)
        # map pref in [-1,1] to per-class gains in [0,1] with dead-zone
    ...
    for ep in range(args.epochs):
        ...
        for ii in ep_idx:
            ...
            # Hidden spikes (time-weighted contribution to class counters)
        ...
        frac_active = float(np.mean((hid_c0+hid_c1) > 0))
        mean_abs_pref = float(np.mean(np.abs(pref_epoch)))
        mean_abs_pref_ema = float(np.mean(np.abs(pref_ema)))
        ...
        print(f"            [Hidden diag] mean spk/sample: {mean_hidden_spk:.1f} | active%: {100*frac_active:.1f}% | |pref| (epoch): {mean_abs_pref:.3f} | |pref| (EMA): {mean_abs_pref_ema:.3f}")
