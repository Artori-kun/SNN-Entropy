...
# -*- coding: utf-8 -*-
"""
STDP (unsupervised) HIDDEN + population OUTPUT — v2.2 (constant-sum mapping)
Why this patch?
- Your logs show Val no-spike% rising (up to ~97%), and |pref| small (~0.1).
- Previous mapping could yield too-weak drive to outputs when pref is small.
Fixes:
1) Constant-sum mapping per hidden neuron: keep (w0 + w1) = W_total; redistribute by preference p in [-1,1]:
       w0 = 0.5*W_total*(1 - p),  w1 = 0.5*W_total*(1 + p)
   => even if |p| is small, outputs still receive sufficient excitation.
2) Output is made more excitable by default (lower v_th_o, a little bias, weaker inhibition, longer synaptic tau).
3) Slightly higher hidden activity (lower v_th_h, milder inhibition).
4) Keep EMA preference + bootstrap in first epochs. No dead-zone by default.
5) Diagnostics for both hidden and output.
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="STDP hidden + population output (v2.2 const-sum)")
    # Data
    ...
    # Hidden->Out constant-sum mapping
    p.add_argument("--w_out_total", type=float, default=1.2, help="Per-hidden total outgoing weight to BOTH class populations")
    ...
    p.add_argument("--bootstrap_epochs", type=int, default=3, help="Use pref_epoch directly for first K epochs")
    ...
    p.add_argument("--report", type=str, default="./stdp_hidden_v22_report")
    return p.parse_args()
...
def train_and_eval(X, y, C, T, args):
    ...
    def update_out_weights_const_sum(pref):
        # Clamp pref to [-1,1]
        p = np.clip(pref, -1.0, 1.0)
        # Constant-sum weights per hidden neuron
        w0_h = 0.5 * args.w_out_total * (1.0 - p)  # to class 0 population
        w1_h = 0.5 * args.w_out_total * (1.0 + p)  # to class 1 population
    ...
    for ep in range(args.epochs):
        ...
        # Preference and mapping
        pref_epoch = (hid_c1 - hid_c0) / (hid_c1 + hid_c0 + 1e-6)
        if ep < args.bootstrap_epochs:
            ...
            pref_ema = (1 - alpha) * pref_ema + alpha * pref_epoch
            pref_hat = pref_ema
        update_out_weights_const_sum(pref_hat)
        ...
        print(f"            [Hidden diag] mean spk/sample: {mean_hidden_spk:.1f} | |pref_hat| used: {float(np.mean(np.abs(pref_hat))):.3f}")
