...
# -*- coding: utf-8 -*-
"""
STDP HIDDEN + population OUTPUT — v2.3
Adds an optional **online supervised mapping** from Hidden→Output
(perceptron-style on a constant-sum parameterization), keeping Hidden unsupervised STDP.

Why:
- Your logs show |pref| small and acc ≈ 0.5–0.55 even after anti-0.5 patches.
- This variant keeps feature learning unsupervised (STDP) but lets the mapping
  adapt **per sample** with labels, which is simple and stable.

Modes:
- map_mode=epoch_constsum (default): as in v2.2 (EMA preference, epochwise update).
- map_mode=online_perceptron: keep a per-hidden preference parameter p_j∈[-1,1].
  After each training sample with label y∈{0,1}, compute hidden activity s_j (time-weighted sum).
  Update p_j ← clip(p_j + β * s_j * ( +1 if y==1 else -1 ), -1, 1).
  Then rebuild H→Out weights using constant-sum mapping:
      w0 = 0.5*W*(1 - p_j), w1 = 0.5*W*(1 + p_j)

Diagnostics: print hidden and output activity and mapping stats.
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="STDP hidden + population output (v2.3 w/ online mapping option)")
    # Data
    ...
    p.add_argument("--v_th_h", type=float, default=0.56)
    p.add_argument("--tau_mem_h", type=float, default=20.0)
    p.add_argument("--tau_syn_h", type=float, default=12.0)
    p.add_argument("--inh_w_h", type=float, default=1.8)
    p.add_argument("--inh_tau_h", type=float, default=10.0)
    p.add_argument("--refractory_h", type=float, default=6.0)
    p.add_argument("--i_bias_h", type=float, default=0.03)
    ...
    p.add_argument("--wmax_h", type=float, default=1.8)
    ...
    p.add_argument("--delay_h_max", type=int, default=10, help="Max delay (ms) for input->hidden synapses")

    # Output population
    p.add_argument("--out_pop", type=int, default=8)
    p.add_argument("--v_th_o", type=float, default=0.50)
    p.add_argument("--tau_mem_o", type=float, default=20.0)
    p.add_argument("--tau_syn_o", type=float, default=15.0)
    p.add_argument("--inh_w_o", type=float, default=1.2)
    p.add_argument("--inh_tau_o", type=float, default=10.0)
    p.add_argument("--refractory_o", type=float, default=5.0)
    p.add_argument("--i_bias_o", type=float, default=0.03)

    # Hidden->Out mapping (constant-sum)
    p.add_argument("--map_mode", type=str, default="epoch_constsum", choices=["epoch_constsum","online_perceptron"])
    p.add_argument("--w_out_total", type=float, default=1.6, help="Per-hidden total outgoing weight to BOTH class populations")
    p.add_argument("--jitter_out", type=float, default=0.05)
    p.add_argument("--tau_pref", type=float, default=40.0, help="Tau (ms) for time-weighting hidden spikes when building preference")
    p.add_argument("--pref_ema_alpha", type=float, default=0.5, help="EMA factor for epoch_constsum")
    p.add_argument("--bootstrap_epochs", type=int, default=3, help="Use epoch preference directly for first K epochs (epoch_constsum)")
    p.add_argument("--map_beta", type=float, default=0.02, help="Perceptron step-size for online mapping")
    ...
    p.add_argument("--report", type=str, default="./stdp_hidden_v23_report")
    return p.parse_args()
...
def build_network(C, T, dt_ms, args, rng):
    ...
    S_h_out.w = (args.w_out_total / 2.0) + args.jitter_out * (rng.random(len(S_h_out)) - 0.5)
...
def train_and_eval(X, y, C, T, args):
    ...
    if args.map_mode == "epoch_constsum":
        pref_ema = np.zeros(args.hidden, dtype=float)  # EMA preference

    elif args.map_mode == "online_perceptron":
        p_online = np.zeros(args.hidden, dtype=float)  # preference parameter in [-1,1]
    ...
    def set_out_weights_from_pref(pref):
        p = np.clip(pref, -1.0, 1.0)
        w0_h = 0.5 * args.w_out_total * (1.0 - p)
        w1_h = 0.5 * args.w_out_total * (1.0 + p)
        w = S_h_out.w[:]; j_arr = S_h_out.j[:]; i_arr = S_h_out.i[:]
        for idx_syn in range(len(S_h_out)):
            j = j_arr[idx_syn]; i = i_arr[idx_syn]
            base = w0_h[j] if i < Np else w1_h[j]
            w[idx_syn] = base + args.jitter_out * (rng.random()-0.5)
        S_h_out.w = clip(w, 0, args.w_out_total + 2*args.jitter_out)
    ...
    for ep in range(args.epochs):
        ...
        for ii in ep_idx:
            ...
            s_j = np.zeros(args.hidden, dtype=float)
            if ids_h.size > 0:
                wtime = np.exp(-t_rel_h / float(args.tau_pref))
                np.add.at(s_j, ids_h, wtime)  # per-sample hidden activity
                hidden_spk_sum += ids_h.size
                if label==0: np.add.at(hid_c0, ids_h, wtime)
                else:        np.add.at(hid_c1, ids_h, wtime)

            # ONLINE MAPPING (supervised) — per sample
            if args.map_mode == "online_perceptron":
                y_sign = +1.0 if label==1 else -1.0
                p_online = np.clip(p_online + args.map_beta * y_sign * s_j, -1.0, 1.0)
                set_out_weights_from_pref(p_online)
        ...
        # EPOCH mapping (unsupervised preference) — if selected
        if args.map_mode == "epoch_constsum":
            ...
            if ep < args.bootstrap_epochs:
                ...
                pref_ema = (1 - alpha) * pref_ema + alpha * pref_epoch
                pref_hat = pref_ema
            set_out_weights_from_pref(pref_hat)
        ...
        print(f"            [Hidden diag] mean spk/sample: {mean_hidden_spk:.1f}")
