...
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# STDP (teacher-aided) classifier for NPZ spike datasets (Poisson vs Bursty)
# This version accepts EITHER (a) a folder with subfolders 'train'/'val' (old layout)
# OR (b) a SINGLE folder with all .npz files and will split into train/val(/test)
# on the fly using stratified random split.
#
# Usage (single-folder):
#   python stdp_teacher_stdp_onefolder.py --npz-dir spike_synth/ch32/trainval \
#     --val-split 0.2 --test-split 0.0 --seed 0 --balance-train --balance-val
#
# Requires: brian2
...
def main():
    ...
    parser.add_argument("--npz-dir", required=True, help="Either folder with train/val or single folder with all .npz")
    parser.add_argument("--x-key", default="x"); parser.add_argument("--label-key", default="label")
    parser.add_argument("--channels", type=int, default=32)
    parser.add_argument("--time", type=int, default=100)
    parser.add_argument("--dt-ms", type=float, default=1.0)
    parser.add_argument("--epochs", type=int, default=20)
    parser.add_argument("--out-pop", type=int, default=8)
    ...
    parser.add_argument("--val-split", type=float, default=0.2, help="if single folder, fraction for validation")
    parser.add_argument("--test-split", type=float, default=0.0, help="if single folder, fraction for test (unused here)")
    parser.add_argument("--seed", type=int, default=0)
    # neuron
    parser.add_argument("--v-th", type=float, default=0.48)
    parser.add_argument("--v-rest", type=float, default=0.0)
    parser.add_argument("--v-reset", type=float, default=0.0)
    parser.add_argument("--tau-m", type=float, default=20.0)
    parser.add_argument("--tau-e", type=float, default=10.0)
    parser.add_argument("--tau-i", type=float, default=10.0)
    parser.add_argument("--t-ref", type=float, default=2.0)
    parser.add_argument("--i-bias", type=float, default=0.12)
    # STDP
    ...
    parser.add_argument("--wmax", type=float, default=3.0)
    parser.add_argument("--w-init", type=float, default=0.6)
    parser.add_argument("--w-norm", type=float, default=1.0)
    # inhibition
    parser.add_argument("--inh-w", type=float, default=0.4)
    parser.add_argument("--cross-inh-w", type=float, default=0.0)
    # teacher
    parser.add_argument("--teacher-i", type=float, default=0.6)
    parser.add_argument("--teach-ms", type=float, default=25.0)
    parser.add_argument("--teach-delay", type=float, default=0.0, help="delay before teacher drive starts (in ms)")
    # readout
    parser.add_argument("--readout-tau", type=float, default=30.0)
    parser.add_argument("--race-N", type=int, default=3)
    # misc
    parser.add_argument("--report", action="store_true")
    args = parser.parse_args()

    # Decide layout: subfolders or single folder
    p = Path(args.npz_dir)
    has_sub = (p/"train").exists() and (p/"val").exists()
    if has_sub:
        # old layout
        Xtr, ytr = load_all_npz(p/"train", x_key=args.x_key, label_key=args.label_key)
        Xva, yva = load_all_npz(p/"val",   x_key=args.x_key, label_key=args.label_key)
    else:
        # single folder: load and stratify split
        Xall, yall = load_all_npz(p, x_key=args.x_key, label_key=args.label_key)
        idx_tr, idx_va, idx_te = stratified_split_indices(yall, val_split=args.val_split, test_split=args.test_split, seed=args.seed)
        Xtr, ytr = Xall[idx_tr], yall[idx_tr]
        Xva, yva = Xall[idx_va], yall[idx_va]

    C, T = Xtr.shape[1], Xtr.shape[2]
    print(f"Loaded train {len(Xtr)}, val {len(Xva)} | shape=({C},{T}) | layout={'subfolders' if has_sub else 'single-folder'}")
